...
export default async ({ req, res, log, error }) => {
  ...
  const messagesToSend = await databases.listDocuments(
    ...
    [Query.limit(10000), Query.equal('sent', false), Query.equal('test', test), Query.select(['$id', 'senderName', 'senderEmail', 'receiverName', 'receiverEmail', 'sonnet'])]
  );
  ...
  for (const message of messagesToSend.documents) {
    try {
      const { senderName, senderEmail, receiverName, receiverEmail, sonnet } = message;
